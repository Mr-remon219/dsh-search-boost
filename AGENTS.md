...
- Import `index.js`, build a mock `ctx` exposing `web.registerSearchProvider` + `web.registerFetchProvider`, `tools.register`, `systemPrompt.section` + `systemPrompt.variable`, `effect(disposer)`, `timeout(ms)`, and `get(name)`, then call `apply(ctx, {})`. This captures the registered providers + tools + prompt variables; invoke `provider.search({query,maxResults})`, `provider.fetch({url})`, or a tool's `execute(args)` directly. Every registration returns a disposer wired into `ctx.effect` (Cordis teardown).
