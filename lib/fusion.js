...
//
// Two layers, switched with /web_change (see lib/layer.js):
//   - TIER_ENGINES        (api layer, default): the full pool — free legs plus
//                          the keyed Tavily / Brave / Exa APIs when keys exist.
//   - TIER_ENGINES_FREE   (free layer): live-probed keyless pool — bing /
//                          exa-free / googlenews / yahoo (no agy; ddg is
//                          domain-search only — flaky on general HTML scrape).
// The active layer stops keyed engines from ever being dialed in free mode.
export const TIER_ENGINES = {
  simple: ['bing', 'ddg', 'yahoo', 'exa-free', 'googlenews'],
  medium: ['bing', 'ddg', 'yahoo', 'exa-free', 'googlenews', 'antigravity', 'tavily'],
  complex: ['bing', 'ddg', 'yahoo', 'exa-free', 'googlenews', 'antigravity', 'tavily', 'brave', 'exa'],
}
export const TIER_ENGINES_FREE = {
  simple: ['bing', 'exa-free', 'googlenews', 'yahoo'],
  medium: ['bing', 'exa-free', 'googlenews', 'yahoo'],
  complex: ['bing', 'exa-free', 'googlenews', 'yahoo'],
}

/** Domain-restricted search (x_search fallback). DDG honors site:; yahoo returns x.com URLs. */
export const FREE_DOMAIN_ENGINES = ['yahoo', 'ddg', 'bing']
...
const ENGINE_WEIGHT = { bing: 1.0, ddg: 1.0, yahoo: 1.0, googlenews: 0.95, antigravity: 1.0, 'exa-free': 1.1, tavily: 1.2, exa: 1.2, brave: 1.1 }
...
export const SECONDARY_VARIANTS = {
  free: ['bing', 'exa-free', 'googlenews', 'yahoo'],
  api: ['bing', 'ddg', 'yahoo', 'exa-free', 'googlenews', 'tavily'],
}
 ...
 */
export async function fusedSearch({ query, queries, engines, maxResults = 6, includeDomains = [], excludeDomains = [], recency, tier = 'auto', layer = null, runOne, signal }) {
