...
// Free HTML scrapes run on every tier. agy (45s CLI) joins from medium up so
// a trivial lookup is not blocked on a local model. Keyed engines join per tier.
...
export function searchCacheKey({ query, queries = [], engines = [], includeDomains = [], excludeDomains = [], recency = null, maxResults, tier }) {
 ...
 */
export async function fusedSearch({ query, queries, engines, maxResults = 6, includeDomains = [], excludeDomains = [], recency, tier = 'auto', runOne, signal }) {
  const started = Date.now()
  const resolvedTier = tier === 'auto' ? estimateComplexity(query) : tier
  const engineNames = (engines ?? TIER_ENGINES[resolvedTier] ?? TIER_ENGINES.simple).filter(Boolean)
