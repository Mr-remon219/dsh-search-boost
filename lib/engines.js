...
export const ENGINE_ORDER = ['bing', 'ddg', 'yahoo', 'googlenews', 'exa-free', 'antigravity', 'tavily', 'brave', 'exa']
...
// Windows undici defaults to IPv6-first DNS; some search hosts time out on v6.
...
function htmlFetch(url, init = {}) {
  return fetch(url, { ...init, agent: ipv4Agent })
}
...
function runAgy(prompt, schemaJson, timeoutMs, signal) {
  return new Promise((resolve, reject) => {
    ...
    execFile('agy', args, { timeout: timeoutMs, signal, maxBuffer: 8 * 1024 * 1024, windowsHide: true }, (err, stdout, stderr) => {
      if (err) {
...
// ---------- Google News RSS (free, keyless) ----------
async function googleNewsSearch(query, count, opts) {
  ...
  for (const m of xml.matchAll(/<item>[\s\S]*?<title>([\s\S]*?)<\/title>[\s\S]*?<link>([\s\S]*?)<\/link>/gi)) {
    const title = collapseSpace(decodeHtml(stripTags(m[1].replace(/<!\[CDATA\[|\]\]>/g, ''))))
    const u = m[2].trim().replace(/<!\[CDATA\[|\]\]>/g, '')
    if (!title || !u.startsWith('http')) continue
    hits.push({ title, url: u, snippet: '', published: null })
    if (hits.length >= count) break
  }
  if (hits.length === 0) throw new Error('googlenews: 0 hits')
  return hits
}
